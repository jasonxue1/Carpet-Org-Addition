...
public class FishingAction extends AbstractPlayerAction {
    ...
    public void tick() {
        ...
        if (pickFishingRod()) {
            // 检查玩家是否抛出钓竿
            FishingBobberEntity fishHook = this.fakePlayer.fishHook;
            if (fishHook == null) {
                // 检查计时器是否清零
    ...
    private boolean pickFishingRod() {
        // 如果玩家手上有钓鱼竿，无需切换
        if (this.fakePlayer.getMainHandStack().isOf(Items.FISHING_ROD) || this.fakePlayer.getOffHandStack().isOf(Items.FISHING_ROD)) {
            return true;
        }
        // 从物品栏拿取钓鱼竿
        ...
        for (int i = 0; i < inventory.getMainStacks().size(); i++) {
            if (inventory.getStack(i).isOf(Items.FISHING_ROD)) {
                ...
                if (this.fakePlayer.getMainHandStack().isOf(Items.FISHING_ROD)) {
                    return true;
                }
            }
        }
    ...
    private void switchInventory() {
        if (this.fakePlayer.getOffHandStack().isOf(Items.FISHING_ROD)) {
            swapHands();
        }
        PlayerInventory inventory = this.fakePlayer.getInventory();
        // 查找物品栏内的非钓鱼竿物品
