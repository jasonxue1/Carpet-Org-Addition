...
public class StonecuttingAction extends AbstractPlayerAction {
    ...
    private boolean tryMoveItem(StonecutterScreenHandler screenHandler, Item item) {
        for (int index = 2; index < screenHandler.slots.size(); index++) {
            ...
            } else if (CarpetOrgAdditionSettings.fakePlayerCraftPickItemFromShulkerBox.get() && InventoryUtils.isOperableSulkerBox(itemStack)) {
                // 从潜影盒中查找指定物品
    ...
    public ArrayList<MutableText> info() {
        ...
        try {
            ...
            StonecuttingRecipe stonecuttingRecipe = world.getRecipeManager()
                    .getAllMatches(RecipeType.STONECUTTING, input, world).get(this.button).value();
            // 获取与配方对应的物品
