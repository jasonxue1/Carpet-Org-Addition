...
public class PlayerManagerCommand extends AbstractServerCommand {
    ...
    public static void loadSafeAfk(ServerPlayerEntity player) {
        if (player instanceof EntityPlayerMPFake) {
            WorldFormat worldFormat = new WorldFormat(player.getEntityWorld().getServer(), null);
            File file = worldFormat.file(SAFEAFK_PROPERTIES);
            // 文件必须存在
            if (file.isFile()) {
                Properties properties = new Properties();
                ...
                try {
                    ...
                    MessageUtils.broadcastMessage(player.getEntityWorld().getServer(), builder.build());
